...
//  var url = "https://docs.google.com/spreadsheets/d/1C5YZ2Lt903A-YGguYQH02JtL9vxs66sMydcD7BeZFJ4/edit#gid=0";
  ...
  for (var i = 0; i < values.length; i++) {
    if (values[i][2] == userInfo.email) {
      ...
      if(userInfo.dplan_approved_date!=values[i][6]){
        ws.getRange(i+1,6+1).setValue(userInfo.dplan_approved_date);
      }
      if(userInfo.qual_approved_sem!=values[i][7]){
        ws.getRange(i+1,7+1).setValue(userInfo.qual_approved_sem);
      }
  ...
  if (!flag){ 
    ws.appendRow([userInfo.firstname,userInfo.lastname,userInfo.email,
                  userInfo.reviewyear,userInfo.advisor,userInfo.first_sem_date,
                  userInfo.dplan_approved_date, userInfo.qual_approved_sem,
...
//    var email = userInfo.email;
    ...
    if (getFolders(folder,email)==false){
...
//  var url = "https://docs.google.com/spreadsheets/d/1C5YZ2Lt903A-YGguYQH02JtL9vxs66sMydcD7BeZFJ4/edit#gid=0";
  ...
  for (var i = 0; i < values.length; i++) {
    if (values[i][2] == userInfo.email) {
      ...
      if(values[i][6]!=""){
        userInfo.dplan_approved_date = values[i][6].toISOString().slice(0,10);
      }
      
      if (values[i][7]!=""){
        userInfo.qual_approved_sem = values[i][7];
      }
      else{
